  ...
  const handleSignup = async () => {
    ...
    const res = await axios.post(URL_USER_SVC, { username, password }).catch((err) => {
      if (err.response.status === STATUS_CODE_CONFLICT) {
        //duplicate username detected, Error code: 409
        ...
        >
          ...
          <Box component="form" noValidate sx={{ mt: 1 }}>
            ...
            <Box display={"flex"} flexDirection={"column"} justifyContent={"flex-end"}>
              <Button variant="contained" sx={{ mt: 3, mb: 2 }} onClick={handleSignup}>
                Sign up
              </Button>
              <Grid container>
                <Grid item xs>
          ...
          <Dialog open={isDialogOpen} onClose={closeDialog}>
            ...
            <DialogActions>
              {isSignupSuccess ? <Button href="/login">Log in</Button> : <Button onClick={closeDialog}>Done</Button>}
            </DialogActions>
          </Dialog>
        </Box>
      </Container>
